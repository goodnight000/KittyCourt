 ...
 * LLM Retry Handler
 *
 * Generic retry handler for single-model LLM API calls with:
 ...
 * @param {string} [config.primaryModel] - Optional model label for metadata/logging
...
async function callLLMWithRetry(config, options = {}) {
    ...
    const modelLabel = primaryModel || 'default';
    let lastError = null;
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            // Add metadata for debugging consistency
            validated._modelUsed = modelLabel;
            validated._usedFallback = false;

            if (onSuccess) {
                onSuccess(validated, primaryModel || null);
            }
