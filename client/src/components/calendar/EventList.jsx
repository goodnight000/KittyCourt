...
const EventCard = memo(({
  ...
  const showOnboarding = !liteMode && isFirstPlanButton && showPlanButton && !hasSavedPlan && !hasDismissedOnboarding
  ...
  const planGlow = isSoon && !liteMode
    ? event.isSecret
  ...
  const showCardShimmer = !liteMode && !prefersReducedMotion && isFirstPlanButton
  const Root = liteMode ? 'div' : Motion.div
  const PlanButton = liteMode ? 'button' : Motion.button

  return (
    <Root
      {...(!liteMode ? { whileTap: { scale: 0.995 } } : {})}
      onClick={onClick}
      className={liteMode
        ? 'group relative w-full rounded-3xl p-[1px] bg-white border border-neutral-200 shadow-soft transition-shadow duration-200 cursor-pointer perf-content-auto contain-paint'
        : `group relative w-full rounded-3xl p-[1px] bg-gradient-to-br ${cardFrame} shadow-soft hover:shadow-soft-lg transition-shadow duration-200 cursor-pointer perf-content-auto contain-paint`}
    >
      {showCardShimmer && (
      ...
      <div className={liteMode
        ? 'relative rounded-[23px] bg-white border border-neutral-200/80'
        : 'relative rounded-[23px] bg-white/90 border border-white/60'}
      >
        <div className="p-3">
          <div className="flex items-start gap-3">
            <div className={liteMode
              ? 'relative w-14 shrink-0 rounded-2xl border border-neutral-200 bg-white shadow-soft'
              : `relative w-14 shrink-0 rounded-2xl p-[1px] bg-gradient-to-br ${dateFrame} shadow-soft`}
            >
        ...
        {showPlanButton && (
          <div className="px-3 pb-3 relative">
            ...
            <PlanButton
              ...
              className={`group/plan relative w-full rounded-full p-[1px] shadow-soft ${!liteMode ? 'hover:shadow-soft-lg transition-shadow duration-200' : ''} ${hasSavedPlan ? 'ring-1 ring-amber-200/30' : ''}`}
            >
              ...
              <span className={`relative z-10 flex items-center justify-between gap-3 w-full h-10 rounded-full px-3.5 bg-gradient-to-r ${planFill} border ${liteMode ? 'border-neutral-200' : 'border-white/70'}`}>
